...
export default {
  ...
  methods: {
    ...
    createChannel: function(){
        this.instrumentList.forEach(element => this.channelList.push({id: element.id , seq: [1], gain:0.5, pan: 0}));
    },

    updateChannel: function() {
        var idArr = this.channelList.map(el => el.id);
        this.instrumentList.forEach(element => (!idArr.includes(element.id)) ? this.channelList.push({id: element.id , seq: [1], gain: 0.5, pan: 0}) : {});
    }, 
